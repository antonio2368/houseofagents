...
pub fn home_help_lines() -> &'static [Line<'static>] {
    static LINES: LazyLock<Vec<Line<'static>>> = LazyLock::new(|| {
        vec![
            ...
            Line::from("  Use loop connections to repeat subgraphs of the DAG."),
            Line::from(""),
    ...
    static SECTIONS: LazyLock<[Vec<Line<'static>>; PIPELINE_TAB_COUNT]> = LazyLock::new(|| {
        ...
        [
            ...
            vec![
                ...
                Line::from("  initial prompt. Use loop connections (o) to repeat"),
                Line::from("  subgraphs of the DAG across multiple passes."),
                Line::from(""),
            ...
            vec![
                ...
                Line::from("    Sub-pipeline targets auto-consume all items"),
                Line::from("    (loop connections are redundant for sub-pipeline scatter)."),
                Line::from("    Regular targets: combine with a loop for full drain."),
            ...
            vec![
                ...
                Line::from(""),
                Line::from("  Loop connections replace the former Iterations setting."),
                Line::from("  The --iterations CLI flag is ignored for pipeline mode."),
                Line::from("  Use o to wire a loop from a downstream block back to"),
                Line::from("  an upstream block; all blocks in between re-run each pass."),
                Line::from(""),
                Line::from(vec![
            ...
            vec![
                ...
                Line::from(""),
                Line::from("  Collection: controls which outputs are fed to finalization."),
                Line::from(
                    "    last_pass — only the latest output per block (deduplicates loop passes)",
                ),
                Line::from("    all_passes — every output including all loop pass variants"),
                Line::from(""),
            ...
            vec![
                ...
                Line::from(""),
                Line::from("  The parent sub-pipeline block can have replicas > 1 —"),
                Line::from("  each replica runs an independent copy of the inner DAG."),
                Line::from("  Sub-pipelines can also be scatter targets."),
                Line::from(""),
