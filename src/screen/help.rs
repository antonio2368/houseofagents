    ...
    static SECTIONS: LazyLock<[Vec<Line<'static>>; PIPELINE_TAB_COUNT]> = LazyLock::new(|| {
        ...
        [
            ...
            vec![
                ...
                Line::from(vec![
                    Span::styled("  s", k),
                    Span::raw(": Open session config to toggle \"Keep Across"),
                ]),
                Line::from("    Iterations\" per session."),
                Line::from(""),
                Line::from("  When Keep Across Iterations is on (default), the"),
                Line::from("  conversation history for that session persists across"),
                Line::from("  all iterations."),
                Line::from(""),
                Line::from("  When off, history is cleared before each iteration —"),
                Line::from("  only the block-to-block data flow is preserved."),
                Line::from(""),
            ],
